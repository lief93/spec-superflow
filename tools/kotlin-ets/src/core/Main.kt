...
fun main(arguments: Array<String>) {
    try {
        ...
        val target = withKotlinFrontend(compilerArgs, entry) { frontend ->
            ...
            val backend = EtsBackend(diagnostics, listOf(stdlib, images, strings, ComposeColorValueRule(), ComposeColorSchemeRule(), ComposeMaterialThemeValueRule(), ComposeAlignmentRule(), ComposeDimensionRule(), ComposeFontRule(fonts), ComposeTextStyleRule(), ComposeEmptyModifierRule(), ComposeWeightRule()) + adapters.rules(), frontend.types)
            if (mode == "page") {
