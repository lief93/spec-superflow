...
fun main(arguments: Array<String>) {
    try {
        ...
        val target = withKotlinFrontend(compilerArgs, entry) { frontend ->
            ...
            val backend = EtsBackend(diagnostics, listOf(stdlib, images, strings, ComposeColorValueRule(), ComposeColorSchemeRule(), ComposeMaterialThemeValueRule(), ComposeTypographyRule(), ComposeAlignmentRule(), ComposeDimensionRule(), ComposeFontRule(fonts), ComposeTextStyleRule(), ComposeTextDecorationRule(), ComposeEmptyModifierRule(), ComposeWeightRule(), CoilImageRequestRule(), ComposeInspectionModeRule(), ComposeLocalContextRule(), ComposeShapeRule(), ComposeButtonColorsRule(), ComposePaddingValuesRule()) + adapters.rules(), frontend.types)
            if (mode == "page") {
