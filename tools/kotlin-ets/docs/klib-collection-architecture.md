...
| `map` / `mapTo` | Official common body candidate; compatibility primitive retained | The official graph also reaches `collectionSizeOrDefault`, capacity construction and its validation/exception closure. This increment does not claim that larger closure. Existing `__etsListMap` support stays in place. |
...
contains the pinned hashes, provenance, results and completed run directories.
Only the dependency KLIB seam, its focused fixture and these documents change
in S1.4. Compose, UI and project adapter files are untouched.
