...
`Iterable.filter`, map, non-indexed flat-map, `firstOrNull` and quantifier paths through
...
| `filter`, `filterTo`, `filterNot`, `filterNotTo` | Official common body reuse | The canonical call graph discovers the four serialized bodies. After official common inlining, only empty construction, traversal and `MutableCollection.add` cross the symbol-bound runtime seam. |
...
| `firstOrNull()` / `firstOrNull(predicate)` | Official common body reuse | The ordinary no-argument body is discovered and emitted with its canonical symbol; the inline predicate overload uses the official common inliner. Exact `List.isEmpty`, `List.get` and iterator symbols cross the typed collection seam. |
| `any` / `all` / `none` | Official common body reuse | Ordinary `any()` and `none()` bodies are discovered and emitted; predicate overloads inline. Exact collection-empty and iterator symbols cross the typed seam, preserving empty identities and short-circuit callback order without `__etsListAny`. |
...
operations needed by a discovered closure. Every call is selected with symbol
...
`tests/klib/collection-map-common/run.mjs` proves the map follow-on. Its discovered
...
discovered from the consumer call, materialized as the sole declaration from its dependency file, and lowered into
`_Collections.ets`; the predicate overload remains inline. Calls to either
discovered symbol bypasses source/JVM compatibility adapters, so the existing
...
and the call site inside the discovered official body. Both cases raise
`UNSUPPORTED_KLIB_DEPENDENCY` before any ETS file is written.

`tests/klib/collection-quantifiers-common/run.mjs` is the representative closure
proof. It exercises ordinary `any()` and `none()` plus predicate `any`, `all`
and `none`, including empty-input identities, zero callbacks, and deterministic
short-circuit callback counts. The emitted modules contain no `__etsListAny`.
Removing the ordinary `any()` body or the exact `Iterator.hasNext` primitive
produces a source-linked `UNSUPPORTED_KLIB_DEPENDENCY` and no ETS output.
...
node tests/klib/collection-quantifiers-common/run.mjs
...
[first-or-null evidence](klib-collection-first-or-null-evidence.json) and
[quantifier evidence](klib-collection-quantifiers-evidence.json) contain the
