...
| P1 | Framework semantics need target-specific structure | `ComposeWidgetAdapter` produces neutral widgets/modifiers and `HarmonyWidgetBackend` produces typed ArkUI nodes | Keep platform adapters, but reuse ordinary expressions, parameter binding and target validation. State, slots and modifiers are not only call-name replacements. |
