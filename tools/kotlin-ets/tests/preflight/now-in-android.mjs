...
try {
  ...
  const collection = run('collect', 'node', [join(root, 'project.mjs'), '--project', project,
    '--module', ':core:designsystem', '--variant', 'demoDebug', '--collect-only', '--work-dir', collected,
    '--offline'], { env: projectEnv });
  ...
  const projectOutput = join(evidence, 'project-attempt.ets');
  const projectReport = join(evidence, 'project-attempt-core-profile.json');
  const projectAttempt = run('compatibility', 'node', [join(root, 'project.mjs'), '--project', project,
    '--module', ':core:designsystem', '--variant', 'demoDebug', '--entry', entry,
    '--unsupported-policy', 'error', '--out', projectOutput, '--preflight-out', projectReport,
    '--work-dir', join(evidence, 'project-attempt'), '--offline'], { env: projectEnv }, 1);
  const compatibility = JSON.parse(projectAttempt.stdout);
  assert.equal(compatibility.code, 'PROJECT_INPUTS_FAILED');
  assert.equal(compatibility.stage, 'compiler-environment');
  assert.match(compatibility.message, /requires Kotlin 2\.1\.20; project uses 2\.1\.10/);
  assert.equal(existsSync(projectOutput), false);
  assert.equal(existsSync(projectReport), false);
  ...
  const baseline = {
    ...
    compiler: { project: inputs.compilerVersion, coverageFrontend: '2.1.20', compatibility },
    ...
    p0Gaps: [
      { category: 'project_dependencies', node: 'compiler_environment',
        responsibleModule: 'tools/kotlin-ets/compiler-environment.mjs', detail: compatibility.message },
  ...
  console.log('PASS Now in Android 5e34fb49: 319-call six-category Core Profile baseline');
  console.log('PASS no target: compiler mismatch and all 16 unsupported Compose calls remain explicit P0 evidence');
} finally {
