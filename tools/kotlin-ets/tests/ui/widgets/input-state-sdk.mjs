...
assert.ok(process.argv[2], 'Pass generated InputStateProfile.ets');
...
const work = mkdtempSync('/private/tmp/kotlin-ets-input-state-sdk-');
...
const generated = join(ets, 'components/InputStateProfile.ets');
...
writeFileSync(join(ets, 'pages/Index.ets'), `import { InputStateProfile, UiState } from '../components/InputStateProfile';
...
struct Index {
  ...
  build() {
    Column() {
      InputStateProfile(this.state, (_action: string): void => {})
    }
  }
}
`);
...
assert.match(compiled, /components\/InputStateProfile\.ts;/);
...
record();
console.log('PASS unmodified generated input-state ETS through actual SDK with typed host invocation');
