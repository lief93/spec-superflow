...
assert.ok(process.argv[2], 'Pass generated StateProfile.ets');
...
const work = mkdtempSync('/private/tmp/kotlin-ets-stateful-props-sdk-');
...
const generated = join(ets, 'components/StateProfile.ets');
copyFileSync(input, generated);
const code = readFileSync(input, 'utf8');
const interfaceTag = code.match(/readonly (__etsInterface_[A-Za-z0-9_]+): boolean;/)?.[1];
assert.ok(interfaceTag, 'Generated source-owned interface tag is required');
writeFileSync(join(ets, 'pages/Index.ets'), `import { StateModel, StateProfile } from '../components/StateProfile';

@Entry
@Component
struct Index {
  @State private lastState: string = 'initial';
  ...
  build() {
    Column() {
      StateProfile({
        title: 'Profile',
        step: 3,
        onState: (value: string): void => { this.lastState = value; },
        subtitle: 'from host',
        model: this.model
      })
      Text(this.lastState)
...
assert.match(compiled, /components\/StateProfile\.ts;/);
...
record();
console.log('PASS unmodified generated stateful-props ETS through actual SDK with typed host invocation');
