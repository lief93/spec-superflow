...
blocks a lower sibling, but does not disable an interactive child. A direct
`detectTapGestures(onTap = ...)` handler maps to the native click event. Raw
pointer handlers, other tap gestures and effectful keys are diagnosed rather
