...
assert.ok(actions.includes('omitted_animation_modifier'));
...
const business = run('business-effect', 'BusinessEffect');
assert.equal(business.result.status, 2);
assert.match(business.report.blockingFailure.message, /LaunchedEffect/);
assert.ok(!business.report.degradations.some(d => d.action === 'omitted_animation_effect'));
const layer = run('static-layer', 'StaticLayer');
assert.equal(layer.result.status, 2);
assert.match(layer.report.blockingFailure.message, /graphicsLayer/);
assert.ok(!layer.report.degradations.some(d => d.action === 'omitted_animation_modifier'));
