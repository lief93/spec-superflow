...
const work = mkdtempSync(join(here, '.work/ir-to-ets-'));
...
const jar = join(work, 'ir-to-ets.jar');
const sources = [
...
run('compile', 'bash', [compiler, ...sources, join(here, 'IrToEtsSeam.kt'), '-d', jar]);
console.log(run('seam', 'java', ['-cp', `${jar}:${classpath}`, 'dev.ets.IrToEtsSeamKt',
  join(here, 'Concatenation.kt'), classpath]));
