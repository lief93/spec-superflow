...
assert.ok(process.argv[2], 'Pass the positive project-adapter evidence directory');
...
const names = ['Models', 'Calls', 'InjectionHost'];
for (const name of names) assert.ok(existsSync(join(generated, `${name}.ets`)), `Missing ${name}.ets`);
const work = mkdtempSync('/private/tmp/kotlin-ets-project-adapter-sdk-');
...
try {
  ...
  rmSync(join(ets, 'generated'), { recursive: true, force: true });
  mkdirSync(join(ets, 'pages'), { recursive: true });
  mkdirSync(join(ets, 'generated'), { recursive: true });
  ...
  for (const name of names) {
    const original = join(generated, `${name}.ets`), copy = join(ets, 'generated', `${name}.ets`);
  ...
  const compiled = inputs.map(input => {
    const record = records.find(row => row[0].endsWith(`/src/main/ets/generated/${input.name}.ts`) &&
      row[2] === 'esm' && row[6] === 'ets');
    assert.ok(record, `${input.name}.ets omitted from SDK inputs`);
    const proto = join(cache, 'entry/src/main/ets/generated', `${input.name}.protoBin`);
  ...
  console.log('PASS: Harmony SDK compiled injected and constructed project state-holder references');
} catch (error) {
