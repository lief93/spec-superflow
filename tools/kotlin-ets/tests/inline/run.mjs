...
const core = ['Frontend.kt', 'OfficialLowerings.kt', 'ExpectedNullability.kt', 'LibraryInlining.kt', 'BinaryBodies.kt', 'LocalDeclarations.kt', 'ForLoops.kt'].map(file => join(root, 'src/core', file)).filter(existsSync);
run('evidence-build', 'bash', [compiler, ...core, join(root, 'src/core/Contract.kt'), join(root, 'src/target/Tree.kt'), join(here, 'InlineEvidence.kt'), '-d', evidence]);
