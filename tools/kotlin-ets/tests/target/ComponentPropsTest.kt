...
fun checkComponentPropsContract() {
    ...
    val child = EtsClass("Child", listOf(label, changes, changed, childBuild), at, component = true, exported = true)
    ...
    val invocation = EtsUiComponent(EtsReference(child.symbol), mapOf("label" to messageValue))
    ...
    check("Child({ label: this.message })" in code)
    ...
    fun withLabel(field: EtsField) = child.copy(members = listOf(field, changes, changed, childBuild))
    ...
    rejected(child.copy(members = listOf(label, changes, changed.copy(returnType = EtsTypes.NUMBER), childBuild)))
    ...
    EtsValidator().validate(program(caller = withCall(invocation.copy(properties = emptyMap()))))
    val named = changed.copy(parameters = listOf(EtsParameter(EtsSymbol("changed:name", "name", EtsTypes.STRING, at))))
    EtsValidator().validate(program(child.copy(members = listOf(label, changes, named, childBuild))))
    System.getenv("KOTLIN_ETS_COMPONENT_FIXTURE")?.let { java.io.File(it).writeText(code) }
    println("Component props: typed bindings, dependency traversal, observers and 14 rejection cases passed")
}
