...
for (const [name, reason] of [['External', /external inherited declarations/], ['Covariance', null],
  ...
  const diagnostic = JSON.parse(run(name, 'java', ['-cp', `${proof}:${cp}`, 'dev.ets.MainKt', '--mode', 'language',
    '--classpath', cp, '--out', output, input], reason === null ? 0 : 2));
  if (reason === null) {
    assert.equal(diagnostic.ok, true); assert.equal(existsSync(output), true); result.formerNegatives.push(name); continue;
  }
