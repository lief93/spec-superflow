...
for (const input of fixtures.filter(path => path.includes('/Unsupported'))) {
  ...
  if (name === 'UnsupportedVariance.kt' || name === 'UnsupportedMultipleBounds.kt') {
    ...
    const entry = name === 'UnsupportedVariance.kt' ? 'variant' : 'multiple';
    assert.equal(vm.runInNewContext(js + `\n${entry}({ read() { return 7; } })`, { exports: {} }, { timeout: 1000 }), 7);
    continue;
  }
...
console.log('PASS focused 40 same-input JVM/host cases, generic-member and unchanged variance/multiple-bound positives, one source-linked negative and JVM/frontend cycle rejection; not a public launcher or SDK run');
