...
const work = mkdtempSync(join(tmpdir(), 'kotlin-ets-varargs-'));
...
const expected = run('jvm', `${java}/bin/java`, ['-cp', `${cp}:${jar}`, 'varargs.OracleKt']).split('\n');
assert.deepEqual(expected, ['12', '7234', 'second', '0']);
...
const actual = ['singleSpread', 'mixed', 'model', 'empty'].map(name => String(context.exports[name]()));
assert.deepEqual(actual, expected);
console.log('PASS JVM/ETS varargs: spread copy, ordered segments, constructors and empty arguments');
