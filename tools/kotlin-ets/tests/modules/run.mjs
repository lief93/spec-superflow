...
assert.deepEqual(imports.flatMap(s => s.importClause.namedBindings.elements.map(n =>
  ...
  ['./Model', 'Counter', undefined], ['./Numbers', 'bump', undefined], ['./Numbers', 'label', undefined],
]);
...
const collision = run('filename-collision', 'bash', [cli, '--mode', 'language', '--out-dir', collisionOutput, ...sources, colliding], 1);
assert.match(collision, /filenames collide/);
...
writeFileSync(join(work, 'result.json'), JSON.stringify({ passed: true, output, genericOutput, implementation, sourceInputs,
  checks: ['JVM differential', 'per-file same-name methods', 'function and type imports', 'helper dependency closure',
