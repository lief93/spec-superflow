...
assert.ok(process.argv[2], 'Pass successful generic-methods/run.mjs evidence after main freeze and SDK slot grant');
...
const work = mkdtempSync('/private/tmp/kotlin-ets-generic-methods-sdk-');
...
assert.deepEqual(manifest.moduleCoverage.filter(module => module.kind === 'interface-only').map(module => module.name), ['MethodPort.ets', 'Readable.ets']);
...
record();
console.log(`PASS actual ETS SDK: ${modules.length} unchanged generic method modules, checked runtime/interface-only inputs and ABC/HAP output`);
