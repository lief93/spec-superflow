...
const paths = [
  ...
  'tests/modules/generic-heritage/Fixture.kt', 'tests/modules/bounded-receivers/BoundedFixture.kt',
  'tests/modules/bounded-receivers/BoundedContract.kt',
];
...
const result = run('contract', 'java', ['-cp', `${cp}:${join(work, 'test.jar')}`, 'dev.ets.BoundedContractKt', manifest.output]);
