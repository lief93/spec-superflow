...
const work = mkdtempSync(join(here, '.work/ui-contract-'));
...
const paths = [
  ...
  'tests/modules/UiModuleFixture.kt', 'tests/modules/UiModuleContract.kt', 'tests/modules/UiModuleHeritage.kt',
];
...
const result = run('contract', 'java', ['-cp', `${cp}:${join(work, 'test.jar')}`, 'dev.ets.UiModuleContractKt', manifest.output]);
