...
assert.ok(process.argv[2], 'Pass successful overloads/run.mjs evidence after main freeze and SDK slot grant');
...
const work = mkdtempSync('/private/tmp/kotlin-ets-overloads-sdk-');
...
assert.deepEqual(manifest.moduleCoverage.filter(module => module.kind === 'interface-only').map(module => module.name), []);
...
record();
console.log(`PASS actual ETS SDK: ${modules.length} unchanged overload modules, checked runtime/interface-only inputs and ABC/HAP output`);
