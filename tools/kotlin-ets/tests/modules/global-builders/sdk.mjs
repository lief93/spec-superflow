...
assert.ok(process.argv[2], 'Pass successful global-builders/run.mjs evidence after main freeze and SDK slot grant');
...
const work = mkdtempSync('/private/tmp/kotlin-ets-global-builders-sdk-');
...
assert.ok(manifest.moduleCoverage.every(module => module.kind === 'runtime'), 'Global builder fixture must retain every runtime module');
...
record();
console.log(`PASS actual ETS SDK: ${modules.length} unchanged global-builder/model/value modules, checked runtime inputs and ABC/HAP output`);
