// Run only after all shared production writers freeze and main grants the CLI slot.
...
assert.ok(process.argv[2], 'Pass a completed focused r2 work directory');
...
const inputs = readdirSync(producer).filter(name => name.endsWith('.jar')).map(name => join(producer, name))
  .concat(readdirSync(here).filter(name => name.endsWith('.kt')).map(name => join(here, name))).map(path => ({ path, sha256: hash(path) }));
...
const results = [];
for (const [name, jars] of [['combined', ['combined.jar']], ['second-jar', ['entry.jar', 'helper.jar']]]) {
  ...
  const expected = run(`${name}-jvm`, 'java', ['-cp', `${classpath}:${oracle}`, 'genericconsumer.OracleKt']).trim().split('\n');
  assert.deepEqual(expected, JSON.parse(readFileSync(join(producer, 'complete.json'), 'utf8')).expected);
...
for (const [name, jars, source, message] of [
  ['signature', ['signature.jar'], 'Application.kt', /JVM binary metadata contains no serialized IR/],
  ['missing-body', ['entry.jar', 'signature-helper.jar'], 'Application.kt', /missing serialized IR body for dependency.*helper/],
  ['missing-jar', ['entry.jar'], 'Application.kt', /unlinked serialized dependencies:.*helper/],
  ['missing-source', ['entry.jar', 'no-source-helper.jar'], 'Application.kt', /no SourceFile attribute.*no-source-helper.jar/],
  ['reified', ['Reified.jar'], 'ReifiedApplication.kt', /reified/],
  ['member', ['Member.jar'], 'MemberApplication.kt', /members are unsupported/],
  ['multifile', ['Multifile.jar'], 'MultifileApplication.kt', /unsupported serialized dependency format MULTIFILE_CLASS_PART/],
]) {
  const output = join(work, `${name}.ets`);
...
writeFileSync(join(work, 'complete.json'), JSON.stringify({ positiveCases: 2, negativeCases: 7, jvmTargetPairs: 6, implementationUnchanged: true }));
console.log('PASS public CLI: six JVM/target pairs, seven closed dependency failures; no producer rebuild');
