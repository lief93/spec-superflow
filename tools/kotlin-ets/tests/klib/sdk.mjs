...
assert.ok(process.argv[2], 'Pass the positive KLIB evidence directory');
...
for (const name of ['Application', 'Library', 'Helper']) {
  assert.ok(existsSync(join(generated, `${name}.ets`)), `Missing generated ${name}.ets`);
}
const work = mkdtempSync('/private/tmp/kotlin-ets-klib-sdk-');
...
function seedFiles(directory, prefix = '') {
  return readdirSync(directory).sort().flatMap(name => {
    ...
    return stat.isDirectory() ? seedFiles(full, relative) : [{ path: relative, sha256: hash(full) }];
  });
}
...
function run(name, command, args, cwd) {
  ...
  const evidence = { name, command, args, cwd, status: result.status, signal: result.signal,
...
try {
  ...
  rmSync(join(ets, 'modules'), { recursive: true, force: true });
  ...
  for (const name of ['Application', 'Library', 'Helper']) {
    const original = join(generated, `${name}.ets`);
    const copy = join(ets, 'modules', `${name}.ets`);
    copyFileSync(original, copy);
    modules.push({ name, original, copy, sha256: hash(original) });
    assert.equal(hash(copy), hash(original));
  }
  write('inputs.json', { seed, generated, page: join(here, 'SdkIndex.ets'), modules });
  ...
  const compiled = modules.map(module => {
    const record = records.find(row => row[0].endsWith(`/src/main/ets/modules/${module.name}.ts`) &&
      row[2] === 'esm' && row[6] === 'ets');
    assert.ok(record, `${module.name}.ets omitted from SDK compiler inputs`);
    const proto = join(cache, 'entry/src/main/ets/modules', `${module.name}.protoBin`);
    assert.ok(lstatSync(proto).size > 0, `${module.name}.protoBin is empty`);
    return { name: module.name, record, proto, sha256: hash(proto) };
  });
  ...
  const haps = readdirSync(outputs).filter(name => name.endsWith('.hap')).map(name => join(outputs, name));
  assert.ok(haps.length > 0, 'SDK produced no HAP');
  for (const module of modules) {
    assert.equal(hash(module.original), module.sha256, `${module.name} original changed`);
    assert.equal(hash(module.copy), module.sha256, `${module.name} staged copy changed`);
  }
  assert.deepEqual(seedFiles(seed), openingSeed, 'Read-only SDK seed changed');
  ...
  console.log('PASS: Harmony SDK compiled all three dependency-linked ETS modules and assembled a HAP');
} catch (error) {
