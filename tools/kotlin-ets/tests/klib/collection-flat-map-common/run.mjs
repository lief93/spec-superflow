...
const expected = run('jvm-run', 'java', ['-cp', `${cp}:${oracle}`, 'collectionflatmapcommon.OracleKt']).trim().split('\n');
assert.deepEqual(expected, ['1,1,-1,3,2,-2', '', '1,-2,2,2,3,-3']);
...
console.log(run('lower', 'java', ['-cp', `${cp}:${probe}`, 'dev.ets.collectionflatmapcommontest.LoadKt', work,
  consumer, stdlib]).trim());
const rejected = join(work, 'rejected');
mkdirSync(rejected);
console.log(run('reject', 'java', ['-cp', `${cp}:${probe}`, 'dev.ets.collectionflatmapcommontest.LoadKt', rejected,
  consumer, stdlib, 'reject-append-all']).trim());
const rejection = readFileSync(join(rejected, 'rejection.tsv'), 'utf8').trim();
assert.match(rejection, /^UNSUPPORTED_KLIB_DEPENDENCY\t/);
assert.match(rejection, /MutableCollections\.kt/);
...
const cases = [[1, 0, 2], [], [-2, 3]];
const actual = cases.map(values => Array.from(context.exports.flatMapTrace(values)).join(','));
...
writeFileSync(join(work, 'result.json'), JSON.stringify({ passed: true, expected, actual, implementation, binary,
  producerSourcesAbsent: true, strictHostTypecheck: true, outerEmpty: true, innerEmpty: true,
  callbackCountAndOrder: true, officialBodies: readFileSync(join(work, 'official-bodies.txt'), 'utf8').split('\n'),
...
console.log('PASS flatMap JVM/typed-ETS nested empties, order, and callback semantics');
