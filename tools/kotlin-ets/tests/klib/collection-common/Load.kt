...
fun main(args: Array<String>) {
    ...
    KlibLoader.withKlibModules(KlibModuleSelection(consumer, listOf(consumer), listOf(stdlib))) { session ->
        ...
        if (rejectAppend) {
            ...
            check(failure.diagnostic.source == rejection.callSite && rejection.signature == bindings.append.signature.toString()) {
                rejection
            }
        ...
        check(collectionReplacements.map { it.signature }.toSet() ==
            setOf(constructor, bindings.iterator, bindings.hasNext, bindings.next, bindings.append)
