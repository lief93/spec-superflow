...
const expected = run('jvm-run', 'java', ['-cp', `${cp}:${oracle}`, 'collectioncommon.OracleKt']).trim().split('\n');
assert.deepEqual(expected, ['2,4|1,3', '|', '-2147483648,-2,0|-3,2147483647']);
...
console.log(run('lower', 'java', ['-cp', `${cp}:${probe}`, 'dev.ets.collectioncommontest.LoadKt', work, consumer, stdlib]).trim());
const rejected = join(work, 'rejected');
mkdirSync(rejected);
console.log(run('reject', 'java', ['-cp', `${cp}:${probe}`, 'dev.ets.collectioncommontest.LoadKt', rejected, consumer,
  stdlib, 'reject-append']).trim());
const rejection = readFileSync(join(rejected, 'rejection.tsv'), 'utf8').trim();
assert.match(rejection, /^UNSUPPORTED_KLIB_DEPENDENCY\t/);
assert.match(rejection, /js\/builtins\/Collections\.kt/);
assert.match(rejection, /Consumer\.kt/);
assert.match(rejection, new RegExp(stdlib.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
...
const actual = cases.map(values =>
  `${Array.from(context.exports.filterEven(values)).join(',')}|${Array.from(context.exports.filterOdd(values)).join(',')}`);
assert.deepEqual(actual, expected);
assert.doesNotMatch(code, /__etsListFilter|kotlin\.js/);
...
writeFileSync(join(work, 'result.json'), JSON.stringify({ passed: true, expected, actual, implementation, binary,
  producerSourcesAbsent: true, strictHostTypecheck: true, officialBodies: ['filter', 'filterTo', 'filterNot', 'filterNotTo'],
...
console.log('PASS filter/filterNot JVM/typed-ETS parity without direct filter helper');
