...
VALIDATION_SCHEMA = "behavior-contract-validation.v2"
REQUIRED_SECTIONS = {
    "schema_version",
    "identity",
    "scope",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate a source-anchored frontend behavior contract and evidence."
    )
    ...
    parser.add_argument("--target", type=Path, help="Target root containing the named scenario evidence files")
...
def validate_runtime_evidence(contract, results, target, closure):
    """Validate platform-neutral, source-bound scenario evidence under one target root."""
    if target is None:
        return ['runtime execution evidence requires --target'], set()
    errors, executed = [], set()
    try:
        target = Path(target)
        ...
        scenarios = {item['id']: item for item in contract.get('scenarios', [])}
        for item in results.get('scenarios', []):
            scenario_id = item.get('id')
            matched = False
            for name in item.get('evidence', []):
                if not isinstance(name, str) or not name or Path(name).is_absolute() or '..' in Path(name).parts:
                    raise ValueError(f'unsafe scenario evidence path: {name!r}')
                path = target / name
                if path.is_symlink() or not path.is_file() or target not in path.resolve().parents:
                    raise ValueError(f'scenario evidence is missing or outside target: {name}')
                record = json.loads(path.read_text(encoding='utf-8'))
                if (record.get('schema_version') != 'behavior-scenario-evidence.v1'
                        or record.get('scenario_id') != scenario_id
                        or record.get('source_revision') != contract.get('identity', {}).get('source_revision')
                        or record.get('target_revision') != results.get('target_revision')
                        or record.get('scope_id') != closure.get('scope', {}).get('id')
                        or record.get('execution_status') != 'executed'
                        or record.get('verdict') != 'pass'):
                    raise ValueError(f'scenario evidence is stale, unexecuted, or mismatched: {name}')
                matched = True
            if matched and scenario_id in scenarios:
                executed.add(scenario_id)
            else:
                errors.append(f'runtime execution evidence lacks a passing record for {scenario_id}')
...
def validate(
    ...
    *,
    ...
    if isinstance(targets, list):
        for target_mapping in targets:
            if not isinstance(target_mapping, dict):
                continue
            action_id = target_mapping.get("id")
            if nonempty_string(action_id):
                target_by_action[action_id] = target_mapping
    ...
    for action_id, target_mapping in target_by_action.items():
        status = target_mapping.get("status")
        status_counts[str(status)] += 1
        if status not in TARGET_STATUSES:
            errors.append(f"target mapping {action_id} has invalid status {status!r}")
        if status == "implemented" and not nonempty_string(target_mapping.get("target")):
    ...
    if phase == "target":
        ...
        if scenario_results is None:
            ...
            runtime_errors, executed_scenarios = validate_runtime_evidence(
                contract, scenario_results, target, closure_result)
