...
def extract(group: dict[str, Any], source_root: Path) -> list[str]:
    relative = Path(group["file"])
    if relative.is_absolute() or ".." in relative.parts:
        raise ValueError(f"unsafe source path: {group['file']!r}")
    source_root = source_root.resolve(strict=True)
    source = source_root / relative
    if source.is_symlink() or not source.is_file() or source_root not in source.resolve(strict=True).parents:
        raise ValueError(f"source file is missing, symbolic, or outside root: {group['file']!r}")
...
def scan(
    ...
    return {
        "schema": "behavior-symbol-scan.v1",
        "verdict": "pass"
