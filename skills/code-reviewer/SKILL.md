...
6. Read the canonical `../release-archivist/references/pr-summary.md` owned by the closing flow, then create or update `<change-dir>/pr-summary.md` using its exact structure. If that reference is missing or unreadable, stop instead of reconstructing the summary from memory. Copy every `tasks.md > TDD Test Plan` row into AC Test Evidence with its Requirement, AC, actual command, and result; do not replace exact files/cases with aggregate counts. Record delivered scope, exceptions, and known risks. Preserve the contract's frontend classification and planned UI/Device obligations; implementation-time UI results may be added now, while release-archivist records the fresh final UI and Device Test evidence.
