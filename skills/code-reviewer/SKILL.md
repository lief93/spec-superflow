...
6. Create or update `<change-dir>/pr-summary.md` from `templates/pr-summary.md`. Copy every `tasks.md > TDD Test Plan` row into AC Test Evidence with its Requirement, AC, actual command, and result; do not replace exact files/cases with aggregate counts. Record delivered scope, exceptions, and known risks. Preserve the contract's frontend classification and planned UI/Device obligations; implementation-time UI results may be added now, while release-archivist records the fresh final UI and Device Test evidence.
