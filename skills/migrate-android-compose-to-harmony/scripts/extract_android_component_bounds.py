...
MARKER = "ANDROID_COMPONENT_BOUNDS:"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract one sanitized Android instrumentation component-bounds inventory from stdin."
    )
...
def extract(log: str) -> dict[str, Any]:
    ...
    if len(candidates) != 1:
        raise ExtractionError(f"expected exactly one {MARKER} record; found {len(candidates)}")
    try:
