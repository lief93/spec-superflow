...
- **Batch execution**: keep AC-specific files and test rows under each AC, then add one Batch Verification block per Batch with complete repository-executable RED/baseline, GREEN, and regression commands verified against real project tooling. Evidence granularity stays at one exact Test Case per AC. Execution granularity groups cases with a shared runner, test target, and lifecycle into one invocation; a per-case report maps that grouped result back to every row. Behavior changes use RED and GREEN; coverage-only, characterization, or unchanged regression uses BASELINE PASS and RERUN without manufactured failures. Prefer class, file, suite, or combined task selectors over repeated Gradle `class#method` or equivalent invocations. When RED and baseline cases share a runner, list and run them in one grouped pre-change invocation, not separate commands per case. Then use one grouped post-change invocation per distinct runner for GREEN and affected regression where the tooling supports it. Do not repeat a runner startup per AC or rerun the same command only to populate another evidence row
