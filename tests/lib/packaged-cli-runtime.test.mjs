...
describe('packaged CLI runtime', () => {
  it('installs from the packed artifact and validates an isolated change', () => {
...
#### Scenario: Installed CLI validates an isolated change
...
|---|---|---|---|---|
| Packaged CLI validates change artifacts | Installed CLI validates an isolated change | Use an isolated fixture | Packaged CLI process | The fixture separates package runtime from source checkout state |
## Decisions
### Decision: Use an isolated fixture
- **Choice**: Create a complete Change in a temporary project.
...
Depends on: None
### AC: Installed CLI validates an isolated change
- **Requirement**: Packaged CLI validates change artifacts
- **User-visible**: No
#### File Changes
##### Create \`test/packaged-cli.test.js\`
- **Responsibility**: Exercise the installed CLI against this isolated Change.
- **Add**: Add the package runtime assertion.
- **Used by**: Package verification.
#### TDD Test Plan
| Layer | Platform | Action | Test File | Test Case | Proves |
|---|---|---|---|---|---|
| Integration | Node | Add | \`test/packaged-cli.test.js\` | \`validates isolated change\` | The installed CLI validates complete isolated artifacts outside its source checkout. |
#### TDD Steps
- [ ] Run the installed CLI against the isolated Change.
`);
    writeFileSync(join(change, 'execution-contract.md'), `# Execution Contract
## Intent Lock
Validate an isolated Change with the installed packaged CLI.
## Approved Behavior
...
|---|---|---|---|
| Packaged CLI validates change artifacts | Validate an isolated Change | Run the package runtime integration case | Batch 1 |
## AC Test Matrix
| Requirement | AC | Layer | Platform | Action | Test File | Test Case | Proves |
|---|---|---|---|---|---|---|---|
| Packaged CLI validates change artifacts | Installed CLI validates an isolated change | Integration | Node | Add | \`test/packaged-cli.test.js\` | \`validates isolated change\` | The installed CLI validates complete isolated artifacts outside its source checkout. |
## Design Constraints
Use the packed CLI and an isolated temporary project.
## Task Batches
### Batch 1
- Validate the isolated Change.
## Test Obligations
